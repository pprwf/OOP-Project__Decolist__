...
public class BottomView extends JPanel {
    ...
    public BottomView () {
        ...
        areaZone.setPreferredSize(new Dimension(175, 220));
        areaZone.setBorder(BorderFactory.createLineBorder(Color.RED));
        buttonZone.setPreferredSize(new Dimension(245, 220));
        buttonZone.setBorder(BorderFactory.createLineBorder(Color.BLUE));
        message.setFont(thai);
        note.setBorder(BorderFactory.createLineBorder(Color.black, 1));
        note.setFont(thai);
        note.setLineWrap(true);
        note.setWrapStyleWord(true);
        scroll.setPreferredSize(new Dimension(180, 175));
        try {
            file = new File("img/room_icon.png");
            roomButton.setButtonImage(ImageIO.read(file));
            System.out.println("Set img for room btn");
